...
enum ProductFeedRoute: Hashable, Identifiable {
    case feedNameRoute
    case feedMemoRoute
    case feedReviewRoute
    var id: Self { self }
}
struct ProductFeedDetail: View {
    let feedId: String
    let appUser: AppUser?
    ...
    @State private var feedRoute: ProductFeedRoute? = nil
                            ...
                            VStack(spacing: 1) {
                                ...
                                if isInfoExpanded {
                                    RowButton(title: "카테고리",
                                              value: feedVm.product?.category?.displayName ?? " ", icon: nil)
                                    Button {
                                        withAnimation(.easeInOut) { isScriptExpanded.toggle() }
                                    } label: {
                                        RowButton(title: "상품 설명",
                                                  value: isScriptExpanded ? " " : clean(feedVm.product?.description ?? ""),
                                                  icon: isScriptExpanded ? "chevron.down" : "chevron.right")
                                    }
                                    if isScriptExpanded {
                                        SubRowButton(value: clean(feedVm.product?.description ?? " "))
                                    ...
                                    if isImagesExpanded {
                                        if let promoKind = feedVm.promoKind {
                                            if promoKind == "product" {
                                                if let url = feedVm.product?.imgUrl {
                                                    RemoteThumb(url: url)
                                                        .padding(.top, 6)
