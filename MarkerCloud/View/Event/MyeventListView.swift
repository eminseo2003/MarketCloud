...
struct MyeventListView: View {
    ...
    private var filteredEvents: [FeedLite] {
                ...
                if filteredEvents.isEmpty {
                    ...
                    LazyVGrid(columns: [GridItem(), GridItem()], spacing: 16) {
                        ForEach(filteredEvents) { event in
                            FeedCard(feed: event, appUser: appUser, selectedMarketID: $selectedMarketID, selectedFeed: $selectedFeed)
                        }
                    }
                    .padding(.horizontal)
